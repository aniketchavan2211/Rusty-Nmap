...
pub async fn ack_scan(target: &str, ports: Vec<u16>, verbose: bool, quiet: bool) {
    println!("[*] ACK Scan: {}", target);
    ...
    let (mut tx, mut rx) = match transport_channel(
        ...
        Err(e) => {
            ...
            if verbose {
                eprintln!("[!] Note: ACK scan requires root privileges on most systems");
            }
            return;
        }
    };
            ...
            Ok((packet, _)) => {
                if let Some(tcp_response) = pnet::packet::tcp::TcpPacket::new(packet) {
                    if tcp_response.get_flags() == TcpFlags::RST {
                        println!("[+] Port {:>5} UNFILTERED", port);
                    } else {
                        if verbose && !quiet {
                            println!("[?] Port {:>5} FILTERED (unexpected response)", port);
                        }
                    }
                }
            }
