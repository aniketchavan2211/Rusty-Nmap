...
pub async fn xmas_scan(target: &str, ports: Vec<u16>, verbose: bool, quiet: bool) {
    println!("[*] Xmas Scan: {}", target);
    ...
    let (mut tx, mut rx) = match transport_channel(
        ...
        Err(e) => {
            ...
            if verbose {
                eprintln!("[!] Note: Xmas scan requires root privileges on most systems");
            }
            return;
        }
    };
    ...
    for port in ports {
        ...
        tcp_packet.set_flags(TcpFlags::FIN | TcpFlags::URG | TcpFlags::PSH);
        tcp_packet.set_urgent_ptr(0xFFFF);
