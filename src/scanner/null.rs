...
pub async fn null_scan(target: &str, ports: Vec<u16>, verbose: bool, quiet: bool) {
    println!("[*] Null Scan: {}", target);
    ...
    let (mut tx, mut rx) = match transport_channel(
        ...
        Err(e) => {
            ...
            if verbose {
                eprintln!("[!] Note: Null scan requires root privileges on most systems");
            }
            return;
        }
    };
    ...
    for port in ports {
        ...
        tcp_packet.set_flags(TcpFlags::default()); // Changed from empty() to default()
